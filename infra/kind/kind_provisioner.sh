...
DEFAULT_KIND_IMAGE="kindest/node:v1.35.0"
...
function setup_kind_cluster() {
  ...
  if ! (yq eval "${CONFIG}" --expression ".networking.disableDefaultCNI = ${KIND_DISABLE_CNI}" \
    ...
    kind create cluster --name="${NAME}" -v2 --retain --image "${IMAGE}" ${KIND_WAIT_FLAG:+"$KIND_WAIT_FLAG"} --config -); then
...
function install_calico {
  ...
  kubectl --kubeconfig="$KUBECONFIG" apply -f "${COMMON_SCRIPTS}/config/calico.yaml"
...
function install_metallb() {
  set +x
  KUBECONFIG="${1}"
  kubectl --kubeconfig="$KUBECONFIG" apply -f "${COMMON_SCRIPTS}/config/metallb-native.yaml"
